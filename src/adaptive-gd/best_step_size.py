...
@app.cell
def _(NDArray, gen_eigenvalues, jnp, np):
    class QuadraticForm:
    ...
    def generate_quadratic_forms(
        ...
        for dim in range(lower_dim, upper_dim + 1, dim_step):
            for null_space_dim in null_space_dims:
                ...
                initial_point_samples = [
                    pt / norm * 10.0 if norm > 0 else pt
...
@app.cell
def _(Path, QuadraticForm, jax, jnp, mo, pl, random):
    def run_experiment_best_step_size(
        experiment_name: str,
        quadratic_forms: list[QuadraticForm],
        initial_point_samples_per_form: list[jnp.array],
        starting_ev_idx: int = 0,
        device_num: int = 1,
    ) -> None:
        with jax.default_device(jax.devices("cpu")[device_num]):
            for f, initial_point_samples in mo.status.progress_bar(
                ...
                eigen_vals = list(f.eigen_vals)
                steps = [eigen_vals.pop(starting_ev_idx)] + random.sample(eigen_vals, k=len(eigen_vals))
                steps = [1 / ev for ev in steps]
                ...
                for init_point_idx, x in mo.status.progress_bar(
                    ...
                    with mo.status.spinner(subtitle="Descending...") as _spinner:
                        for i, lambda_i in enumerate(steps):
                            ...
                            results["loss"].append(f.calculate_function(x))
    ...
    return (run_experiment_best_step_size,)
...
@app.cell
def _(initial_points, quadratic_forms, run_experiment_best_step_size):
    run_experiment_best_step_size(
        experiment_name="best_step_size_min",
...
def _(initial_points, quadratic_forms, run_experiment_best_step_size):
    run_experiment_best_step_size(
        experiment_name="best_step_size_max",
        quadratic_forms=quadratic_forms,
        initial_point_samples_per_form=initial_points,
        starting_ev_idx=-1,
        device_num=0,
...
def _(left_data, pl, right_data, wilcoxon):
    hypotheses_test: pl.DataFrame = (
        ...
        .with_columns(
            ...
            .map_elements(
                lambda x: wilcoxon(x["loss"], x["lossRight"]).pvalue, return_dtype=pl.Float64
            )
            .alias("p_value")
        )
    )
...
def _(hypotheses_test: "pl.DataFrame", pl):
    conv_speed_hypotheses_test = (
        hypotheses_test.explode("iteration", "loss", "lossRight")
        .filter(pl.col("iteration").le(100))
        .group_by("dimension", "kernel_size", "initial_point_index")
        .agg(
